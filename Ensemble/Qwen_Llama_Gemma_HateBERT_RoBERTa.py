...
tokenizer_hatebert = BertTokenizerFast.from_pretrained("GroNLP/hateBERT")
tokenizer_roberta = RobertaTokenizerFast.from_pretrained('roberta-large', add_prefix_space=True)
tokenizer_qwen = AutoTokenizer.from_pretrained("Qwen/Qwen3Guard-Gen-8B", use_fast=True)
...
max_length_test_hatebert = 0
for i in range(len(test_json)):
    tokenized = tokenizer_hatebert(test_json[i]['sentence_tokens'], return_offsets_mapping=True, is_split_into_words=True, padding=False)
    test_json[i]['tokenized_hatebert'] = tokenized
    max_length_test_hatebert = max(len(tokenized['input_ids']), max_length_test_hatebert)
...
for data in test_json:
    ...
    inputs_qwen = tokenizer_qwen(prompt, return_tensors="pt")
    ...
    #hatebert
    labels_hatebert = [-100]
    tokenized_hatebert = data['tokenized_hatebert']
    att_labels_hatebert = tokenized_hatebert['attention_mask'].copy()
    i = -1
    att_labels_hatebert[0] = 0
    for j in range(1, len(tokenized_hatebert['offset_mapping'])):
        ofmap = tokenized_hatebert['offset_mapping'][j]
        if ofmap==(0,0):
            labels_hatebert.append(-100)
            att_labels_hatebert[j] = 0
        elif ofmap[0]==0:
            i+=1
            labels_hatebert.append(data['labels'][i])
        else:
            labels_hatebert.append(-100)
            att_labels_hatebert[j] = 0
    pad_list_hatebert = [0 for p in range(max_length_valid-len(labels))]

    #Llama
    target_sentence_llama = data['sentence_tokens']
    user = f"The number of words in the input sentence is {len(target_sentence)}." + "\n" + f"input: {target_sentence}" + "\n"
    messages = [
        {"role":"system", "content":system},
        {"role":"user", "content":user},
    ...
    inputs_llama = tokenizer_llama(prompt, return_tensors="pt")
    ...
    data_dict_test[str(data['Id'])] = {
        ...
        'inputs_qwen': inputs_qwen,
        'inputs_llama': inputs_llama,
        ...
        'usage':data['usage'],
        'input_ids_hatebert':torch.tensor(tokenized_hatebert['input_ids']+pad_list_hatebert), 
        'attention_mask_hatebert':torch.tensor(tokenized_hatebert['attention_mask']+pad_list_hatebert),
        'attention_labels_hatebert':torch.tensor(att_labels_hatebert+pad_list_hatebert),
    }
...
class EnsembleModel(nn.Module):
    def __init__(self):
        ...
        #Qwen3Guard
        model_name_qwen = "Qwen/Qwen3Guard-Gen-8B"
        self.qwen_tokenizer = AutoTokenizer.from_pretrained(model_name_qwen, use_fast=True)
        base_model = AutoModelForCausalLM.from_pretrained(model_name_qwen, device_map=None)
        lora_dir = "./results/Qwen/"
        self.qwen = PeftModel.from_pretrained(base_model, lora_dir)
        self.qwen.eval()
        ...
        base_model = AutoModelForCausalLM.from_pretrained(model_name_gemma, device_map=None)
        lora_dir = "./results/Gemma/"
        self.gemma = PeftModel.from_pretrained(base_model, lora_dir)
        self.gemma.eval()
        for param in self.gemma.parameters():
            param.requires_grad = False
        self.gemma.to('cuda:0')

        #HateBERT
        self.hatebert = BertForTokenClassification.from_pretrained(
            "./results/HateBERT/"
        )
        self.hatebert.eval()
        for param in self.hatebert.parameters():
            param.requires_grad = False
        self.hatebert.to("cuda:0")

        #Llama
        model_name_llama = "meta-llama/Llama-3.1-8B-Instruct"
        self.llama_tokenizer = AutoTokenizer.from_pretrained(model_name_llama, use_fast=True)
        base_model_llama = AutoModelForCausalLM.from_pretrained(model_name_llama, device_map=None)
        lora_dir = "./results/Llama/"
        self.llama = PeftModel.from_pretrained(base_model_llama, lora_dir)
        self.llama.eval()
        for param in self.llama.parameters():
            param.requires_grad = False
        self.llama.to("cuda:1")
    ...
    def forward(
        ...
        inputs_llama=None,
        inputs_gemma=None,
        input_ids_hatebert=None,
        attention_mask_hatebert=None,
        attention_labels_hatebert=None,
        **kwargs,
    ):
        ...
        #qwen_prediction
        inputs_qwen.to(self.qwen.device)
        with torch.no_grad():
            outputs_qwen = self.qwen.generate(
                **inputs_qwen,
                max_new_tokens=2048,
                do_sample=False,
                pad_token_id=self.qwen_tokenizer.eos_token_id,
            )
        output_text = self.qwen_tokenizer.decode(outputs_qwen[0], skip_special_tokens=True)
            ...
            if pred_dict is not None:
                ...
                if len(pred_result)!=len(pred_roberta):
                    ...
                    for pred in pred_result:
                        pred_qwen.append(pred['label'])

        #HateBERT_prediction
        input_ids_hatebert.to(self.hatebert.device)
        attention_mask_hatebert.to(self.hatebert.device)
        with torch.no_grad():
            outputs_hatebert = self.hatebert(input_ids=input_ids_hatebert, attention_mask=attention_mask_hatebert)
        logits_hatebert = outputs_hatebert.logits.argmax(-1)
        pred_hatebert = logits_hatebert[0][attention_labels_hatebert[0].bool()].tolist()

        #Llama_prediction
        inputs_llama.to(self.llama.device)
        with torch.no_grad():
            outputs_llama = self.llama.generate(
                **inputs_llama,
                max_new_tokens=2048,
                do_sample=False,
                pad_token_id=self.llama_tokenizer.eos_token_id,
            )
        output_text_llama = self.llama_tokenizer.decode(outputs_qwen[0], skip_special_tokens=True)
        generated_llama = output_text_llama.strip()
    
        matched_list_llama = re.findall(r'\{\"result\": \[[^\]]*\]\}' ,generated_llama)
        pred_llama = []
        if len(matched_list_llama)<=1:
            pass
        else:
            matched_text = matched_list_llama[1].replace("}.", "},").replace("\\\'", "\'").replace("\'\"\'", "\'\\\"\'").replace("\'text\'", "\"text\"").replace("\'label\'", "\"label\"")
            matched_text = re.sub("\":\s?\'", r'": "', matched_text)
            matched_text = re.sub("\'[,.]\s?\"label", r'", "label', matched_text)
            matched_text = re.sub("},\s?\'([^{}[]]*\",\s?\"label\")", r'}, {"text": "\1', matched_text)
            matched_text = re.sub("\\\([^u\"])", r'\1', matched_text)
            ...
            if pred_dict is not None:
                ...
                if len(pred_result)!=len(pred_roberta):
                    ...
                    for pred in pred_result:
                        pred_llama.append(pred['label'])
        
        #gemma_prediction
        inputs_gemma.to(self.gemma.device)
        with torch.no_grad():
        ...
        #Prediction
        pref_pred = pred_gemma
        th = 3
        if len(pred_gemma)==0:
            th = 2.5
            pred_gemma = [0 for _ in pred_roberta]
            if len(pred_qwen)==0:
                pref_pred = pred_roberta
            else:
                pref_pred = pred_qwen
        if len(pred_qwen)==0:
            th -= 0.5
            pred_qwen = [0 for _ in pred_roberta]
        if len(pred_llama)==0:
            th -= 0.5
            pred_llama = [0 for _ in pred_roberta]
        logits_list = []
        for p_g,p_q,p_r,p_l,p_h,p_p in zip(pred_gemma, pred_qwen, pred_roberta, pred_llama, pred_hatebert, pref_pred):
            p_sum = p_g+p_q+p_r+p_l+p_h
            if p_sum==th:
                logits_list.append(p_p)
            elif p_sum>th:
                logits_list.append(1)
            else:
...
for data in tqdm(data_list_test):
    ...
    input_ids_hatebert=data['input_ids_hatebert'].unsqueeze(0).to(model.hatebert.device)
    attention_mask_hatebert=data['attention_mask_hatebert'].unsqueeze(0).to(model.hatebert.device)
    attention_labels_hatebert=data['attention_labels_hatebert'].unsqueeze(0).to(model.hatebert.device)
    inputs_qwen=data['inputs_qwen'].to(model.qwen.device)
    inputs_llama=data['inputs_llama'].to(model.llama.device)
    inputs_gemma=data['inputs_gemma'].to(model.gemma.device)
    labels=data['labels'].to(model.roberta.device)
    attention_mask_labels=data['attention_mask_labels'].unsqueeze(0).to(model.roberta.device)
    with torch.no_grad():
        output = model(
            ...
            inputs_llama=inputs_llama,
            inputs_gemma=inputs_gemma,
            input_ids_hatebert=input_ids_hatebert,
            attention_mask_hatebert=attention_mask_hatebert,
            attention_labels_hatebert=attention_labels_hatebert,
        )
