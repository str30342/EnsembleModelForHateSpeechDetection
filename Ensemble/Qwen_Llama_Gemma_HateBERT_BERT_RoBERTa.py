...
for data in test_json:
    ...
    for j in range(1, len(tokenized_bert['offset_mapping'])):
        ...
        if ofmap==(0,0):
            ...
            labels_bert.append(-100)
            att_labels_bert[j] = 0
    pad_list_bert = [0 for p in range(max_length_valid_bert-len(labels_bert))]
...
class EnsembleModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        logits_list = []
        for p_g,p_q,p_r,p_l,p_h,p_b,p_p in zip(pred_gemma, pred_qwen, pred_roberta, pred_llama, pred_hatebert, pred_bert, pref_pred):
            p_sum = p_g+p_q+p_r+p_l+p_h+p_b
            if p_sum==th:
