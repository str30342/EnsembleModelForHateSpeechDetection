    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(pred_gemma)==0: #JSON format error
            if len(pred_qwen)==0:
                logits_list = pred_roberta
            else:
                logits_list = pred_qwen
        elif len(pred_qwen)==0:
            logits_list = pred_gemma
        else:
            logits_list = []
            for p_g,p_q,p_r in zip(pred_gemma, pred_qwen, pred_roberta):
                if p_g==p_q:
                    logits_list.append(p_g)
                else:
...
model.roberta.to("cuda:1")
model.gemma.to("cuda:0")
model.qwen.to("cuda:1")
data_pred = []
